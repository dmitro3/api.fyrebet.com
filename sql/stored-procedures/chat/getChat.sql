...
-- [varchar]  = Error
use casino;
drop PROCEDURE if exists `getChatRoom`;
CREATE PROCEDURE `getChatRoom`(
...
currentFunction:BEGIN
    ...
    IF @chatRoomType = 'PUBLIC' THEN
        ...
        CONCAT('[', GROUP_CONCAT(CONCAT('{"userUUID":"', u.UUID, '", "userId":',u.id,'}')) , ']') as participants,
        (SELECT CONCAT('[', GROUP_CONCAT(CONCAT('{"messageUUID":"', cM.UUID, '", "messageText":"',cM.messageText,'", "userUUID":"',cMU.UUID,'", "avatarUrl":"',avatars.url,'", "username":"',cMU.username,'"}')) , ']') 
    ...
    ELSE
        IF @chatRoomType = 'PRIVATE' THEN
            ...
            group by cRP.chatRoomId, chatRoomUUID, otherPartyAvatar.url limit 1;
        END IF;
    END IF;
