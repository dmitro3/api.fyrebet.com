...
const getChatRoom = async ({ chatRoomUUID, requestingUserId }) => {
    ...
    const queryResults = await db.query('call `getChatThread`(?,?,@error); select @error as procError;', [chatRoomUUID, requestingUserId]);
        ...
        if (Array.isArray(queryResult) && queryResult.length) {
            ...
            if ('chatRoomUUID' in queryResult[0]) {
                ...
                try {
                    ...
                    console.log(e);
                    console.log(chatRoom.messages[26123])
                    console.log(chatRoom.messages[26124])
                    console.log(chatRoom.messages[26125])
                    chatRoom.messages = [];
                }
                return chatRoom;
            }
